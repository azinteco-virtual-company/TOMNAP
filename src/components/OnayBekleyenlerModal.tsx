    ...
    try {
      ...
      const res = await fetch('/api/webhook/siparis', {
        ...
        body: JSON.stringify({
          ...
          tenant_id: seciliFirmaId || 'kanada_shopper_baku',
        }),
      });
      const data = await res.json();
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-base">
                  Gelen Kutusu: Onay Bekleyen Siparişler
                </h3>
              ...
              <p className="text-xs text-amber-100">
                Instagram DM / WhatsApp konuşmasında <strong>#SİPARİŞ</strong> veya <strong>#ONAY</strong> ile tetiklenen sipariş taslakları
              </p>
            </div>
          </div>
          <button
                ...
                <div className="p-4 rounded-xl bg-slate-900 text-slate-200 border border-slate-800">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] bg-slate-800 text-slate-400 px-2 py-0.5 rounded font-mono">
                      {seciliMesaj.kaynak} • {new Date(seciliMesaj.gelis_tarihi).toLocaleTimeString('tr-TR')}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setDuzenlemeForm({ ...duzenlemeForm, instagram_kullanici_adi: e.target.value })}
                    ...
                    <div className="sm:col-span-2 bg-amber-50 p-2.5 rounded-lg border border-amber-200 flex items-center justify-between">
                      ...
                      <span className="text-sm font-bold text-amber-900">
                        {Math.max(0, duzenlemeForm.toplam_tutar - duzenlemeForm.alinan_tutar).toFixed(2)} {duzenlemeForm.para_birimi}
                      </span>
                    </div>
                  </div>
                </div>
