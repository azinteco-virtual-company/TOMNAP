...
export const SiparisTablosu: React.FC<SiparisTablosuProps> = ({
  ...
  const hazirlaniyorSayisi = siparisler.filter(
    (s) => s.lojistik_durumu === 'KANADA_SATINALIM_BEKLIYOR'
  ).length;
  const yoldaSayisi = siparisler.filter(
    (s) =>
      s.lojistik_durumu === 'KANADA_DEPO' ||
      s.lojistik_durumu === 'ULUSLARARASI_KARGO' ||
      s.lojistik_durumu === 'BAKU_DAGITIM_ARKADAS'
      ...
      <div className="p-4 border-b border-slate-100 bg-slate-50 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            onClick={async () => {
              try {
                await apiFetch('/api/siparisler/tumunu-uluslararasi-kargo-yap', { method: 'POST' });
      ...
      <div className="px-4 py-2.5 bg-white border-b border-slate-200/80 flex items-center justify-between gap-3 overflow-x-auto">
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer ${
              seciliLojistik === 'YOLDA' ||
              seciliLojistik === 'ULUSLARARASI_KARGO' ||
              seciliLojistik === 'KANADA_DEPO' ||
              seciliLojistik === 'BAKU_DAGITIM_ARKADAS'
          ...
          <div className={`space-y-3 p-3 ${gorunumTipi === 'kart' ? 'block' : 'block md:hidden'}`}>
            {filtrelenmisSiparisler.length === 0 ? (
              ...
              filtrelenmisSiparisler.map((siparis) => {
                ...
                const temizGorsel =
                  ilkGorsel &&
                  (ilkGorsel.startsWith('/uploads/') ||
                    ilkGorsel.startsWith('data:') ||
                    ilkGorsel.startsWith('http'))
                    ? ilkGorsel
                    : ilkGorsel && ilkGorsel.includes('.svg')
                      ? `/uploads/${ilkGorsel}`
                      : undefined;
                    ...
                    <div className="pt-2.5 border-t border-slate-100 grid grid-cols-2 gap-2 text-xs">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            onDurumGuncelle(siparis.id, { lojistik_durumu: e.target.value as any })
                          }
                        ...
                        <td className="p-3">
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span className="text-[10px] bg-slate-100 text-slate-600 px-1 py-0.5 rounded font-medium">
                              {(siparis.siparis_kaynagi || 'BELİRTİLMEDİ').replace(
                                'INSTAGRAM_',
                                'IG '
                              )}
                            </span>
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    {siparis.gorsel_urlleri &&
                                      siparis.gorsel_urlleri.length > 1 && (
                        ...
                        <td className="p-3">
                          <div className="flex items-center gap-1.5">
                            ...
                            >
                              ...
                              <option value="BAKU_DAGITIM_ARKADAS">BAKU_DAGITIM_ARKADAS</option>
                          ...
                          {(siparis.kanada_takip_kodu || siparis.uluslararasi_kargo_kodu) && (
                            <div className="mt-1 space-y-0.5 font-mono text-[10px] leading-tight">
                              {siparis.kanada_takip_kodu && (
                                <div
                                  className="text-rose-700 font-semibold truncate"
                                  title={`Kanada Takip: ${siparis.kanada_takip_kodu}`}
                                >
                                  🇨🇦 {siparis.kanada_takip_kodu}
                                </div>
                              )}
                              {siparis.uluslararasi_kargo_kodu && (
                                <div
                                  className="text-blue-700 font-semibold truncate"
                                  title={`Kargo Kodu: ${siparis.uluslararasi_kargo_kodu}`}
                                >
                        ...
                        <td className="p-3 max-w-xs">
                          ...
                          {siparis.baku_tahsilat_notu && (
                            <div
                              ...
                              title={siparis.baku_tahsilat_notu}
                            >
                              "{siparis.baku_tahsilat_notu}"
                            </div>
                          )}
