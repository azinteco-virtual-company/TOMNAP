...
4. **KRİTİK — Kargo senkronizasyonu simüle sonuçları gerçek siparişlere yazıyor.**
   ...
   - `POST /api/kargo/senkronize-et` bu sonucu siparişin `lojistik_durumu`'na **uyguluyor**; `TESLIM_EDILDI` dahil (`src/server/services/kargo/kargoMerkezi.ts:84,151`). Nota da "Canlı" yazılıyor.
...
- **Aramex takip:** Gerçek HTTP çağrısı (`https://ws.aramex.net/...TrackShipments`, `src/server/services/kargo/providers/aramex.ts:19-21,156`). Ancak yalnız kullanıcı adı, şifre ve hesap no girilmiş ve `testModu` kapalıysa çalışıyor. Aksi halde **simülasyon** dönüyor (`:128-133, 439+`); KRİTİK bulgu 4'e bakın.
...
| `POST /api/kargo/senkronize-et` (simülasyonda)                                                | PATRON, KANADA_SATINALMA              | Siparişlerin lojistik durumunu simüle sonuçla değiştirir                                                                         | Yok                                                                        | KRİTİK bulgu 4                                                                                                      |
