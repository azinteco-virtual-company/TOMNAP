-- One order note contract for v1 and v2 (Codex R3 F8).
-- No migration creates a physical siparisler.ozel_not column, yet the v2 order RPC wrote
-- it: on a schema without that column every v2 order failed. The note now goes where v1
-- keeps it: the "[TƏLİMAT: …]" tag at the start of baku_tahsilat_notu. Replaces only
-- tomnap_v2_siparis_olustur (body of 20260925100000, owner rule unchanged); no table,
-- column or data changes. The server reads the tag first and a legacy physical column
-- only when there is no tag (src/server/services/siparisFormatlama.ts).
-- Rollback: supabase/rollbacks/20260925160000_not_sozlesmesi.down.sql
...
BEGIN
  ...
  -- The delivery note is the "[TƏLİMAT: …]" tag of baku_tahsilat_notu, as in v1 (Codex R3 F8);
  -- no physical ozel_not column is written. Brackets become parentheses so the tag holds.
  v_not := nullif(btrim(coalesce(p_siparis->>'ozel_not', ''), E' \t\r\n'), '');
  IF v_not IS NOT NULL THEN
    v_not := '[TƏLİMAT: ' || replace(replace(v_not, '[', '('), ']', ')') || ']';
  END IF;
  ...
  INSERT INTO public.siparisler(
    ...
    adet, toplam_tutar, alinan_tutar, para_birimi, finans_durumu, lojistik_durumu, baku_tahsilat_notu,
