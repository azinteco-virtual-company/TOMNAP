-- Rollback for supabase/migrations/20260925160000_not_sozlesmesi.sql.
-- Puts back the owner-rule (20260925100000) body of tomnap_v2_siparis_olustur, which
-- writes the note into a physical ozel_not column (that body needs such a column).
-- Drops nothing. Does nothing when the function is already gone (rollbacks run newest
-- first), so it can run twice.
...
DO $down$ BEGIN
  IF to_regprocedure('public.tomnap_v2_siparis_olustur(text,text,jsonb,jsonb)') IS NULL THEN
    RETURN;
  END IF;
  EXECUTE $m11$
...
END $body$;
$m11$;
  EXECUTE 'REVOKE ALL ON FUNCTION public.tomnap_v2_siparis_olustur(text, text, jsonb, jsonb) FROM PUBLIC, anon, authenticated';
  EXECUTE 'GRANT EXECUTE ON FUNCTION public.tomnap_v2_siparis_olustur(text, text, jsonb, jsonb) TO service_role';
END $down$;
