-- Rollback for supabase/migrations/20260925140000_para_yazma_yetkisi.sql.
-- Puts back the A10 (20260925110000) bodies of tomnap_v2_odeme_kaydet and
-- tomnap_v2_odeme_ters_kayit and the A11 (20260925120000) body of
-- tomnap_v2_kasa_teslimi: SUPER_ADMIN writes again. Drops nothing. A function whose
...
$old$;
  ...
  END IF;
  IF to_regprocedure('public.tomnap_v2_odeme_ters_kayit(text,text,uuid,text)') IS NOT NULL THEN
    EXECUTE $old$
...
END $body$;
$old$;
    EXECUTE 'REVOKE ALL ON FUNCTION public.tomnap_v2_odeme_ters_kayit(text, text, uuid, text) FROM PUBLIC, anon, authenticated';
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.tomnap_v2_odeme_ters_kayit(text, text, uuid, text) TO service_role';
  END IF;
  IF to_regprocedure('public.tomnap_v2_kasa_teslimi(text,text,text,uuid[],numeric,text)') IS NOT NULL THEN
    EXECUTE $old$
...
END $body$;
$old$;
    EXECUTE 'REVOKE ALL ON FUNCTION public.tomnap_v2_kasa_teslimi(text, text, text, uuid[], numeric, text) FROM PUBLIC, anon, authenticated';
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.tomnap_v2_kasa_teslimi(text, text, text, uuid[], numeric, text) TO service_role';
  END IF;
END $down$;
